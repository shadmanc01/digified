...
export default function RootLayout({children}:{children:React.ReactNode}){
  return <html lang="en"><body>
    <header className="topbar"><div className="shell" style={{width:'100%',display:'flex',alignItems:'center',gap:24}}>
      ...
      <nav className="nav"><Link href="/explore">Explore</Link><Link className="hideTablet" href="/following">Following</Link><Link className="hideTablet" href="/cameras">Cameras</Link><Link href="/upload">Upload</Link><Link className="hideTablet" href="/messages">Messages</Link><Link href="/login">Sign in</Link><Link className="join" href="/signup">Join digified</Link></nav>
    </div></header>
