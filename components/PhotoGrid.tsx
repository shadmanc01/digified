...
export default function PhotoGrid({items}:{items:Photo[]}){return <div className="masonry">{items.map(p=><Link className="photoCard" href={`/photo/${p.id}`} key={p.id}>
  <Image src={p.src} alt={p.alt} width={p.aspect==='portrait'?900:1200} height={p.aspect==='portrait'?1200:p.aspect==='square'?1000:800} sizes="(max-width:460px) 100vw,(max-width:760px) 50vw,(max-width:1100px) 33vw,25vw"/>
